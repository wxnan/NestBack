...
class _ItemDetailPageState extends State<ItemDetailPage> {
  ...
  Widget _buildSubcategorySelector() {
    return Consumer2<CategoryProvider, HouseProvider>(
      builder: (context, categoryProvider, houseProvider, _) {
        ...
        if (subcategories.isEmpty && _selectedSubcategoryId != null) {
          return FutureBuilder<String>(
            future: _loadSubcategoryName(_selectedSubcategoryId!),
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return _buildEmptySubcategoryDropdown(categoryProvider, currentHouse);
            },
          );
        }
  ...
  Widget _buildEmptySubcategoryDropdown(CategoryProvider categoryProvider, House? currentHouse) {
    return DropdownButtonFormField<String>(
      value: '',
      decoration: InputDecoration(
        labelText: '二级分类（选填）',
    ...
    return Consumer2<SpaceProvider, HouseProvider>(
      builder: (context, spaceProvider, houseProvider, _) {
        ...
        return DropdownButtonFormField<String>(
          ...
          onChanged: (value) async {
            if (value != null && value != _selectedSpaceId) {
              final targetSpace = spaces.firstWhere((s) => s.id == value);
